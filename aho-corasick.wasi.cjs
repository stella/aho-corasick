...
let __wasmFilePath = __nodePath.join(__dirname, 'aho-corasick.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'aho-corasick.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@stll/aho-corasick-wasm32-wasi')
  ...
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@stll/aho-corasick-wasm32-wasi is installed but is missing aho-corasick.wasm32-wasi.wasm.',
    )
  }
